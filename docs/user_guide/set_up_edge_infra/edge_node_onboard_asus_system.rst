...
#. Use the following command to download the iPXE EFI file and secure boot
   certificates from the provided source.
...
#. Create Boot Directory and copy the ``signed_ipxe.efi`` and ``db.der``
   to the drive.
   ...
   .. code-block:: shell
      ...
      sudo cp db.der /tmp/efidrive/efi/boot/
...
#. Go to **Security** > **Key Management** and append the
   ``Authorized Signature`` with ``db-emt.der``.
