...
* Pre-built Edge Node Software profiles to provision edge nodes with Ubuntu OS
  as well as Edge Microvisor Toolkit immutable container host, enabling
...
* Support for a range of Intel hardware, from Intel Atom® to Intel® Core™,
  Intel® Core™ Ultra, to Intel® Xeon®, Intel® Arc™ Graphics, and Intel®
  Ethernet Network Adapters that serves the diverse and heterogenous compute
...
Version 3.0 provides the following additional features and enhancements on
...
* Open Sourcing of the codebase
    ...
    * All repositories can be cloned and built as an artifact
      (binary file, rpm, or Docker container) * All components have
      and minor versions * Simplified the total number of repos.
    * Each repository have a README.md file.

* Introducing a complete new Developer Documentation.
...
* Edge Node cluster management via CAPI (Cluster API)

   * Cluster providers based on the open cluster API standard, for the
     existing device/infra, RKE cluster provisioning and other flavors like
     the vanilla Kubernetes setup.
   * Support cluster templates/profile based on CAPI, to
...
* Edge Node Zero touch Onboarding and provisioning with pre-registration
   ...
   * pre-register a node with Operating System, Security (Secure Boot and
     Full Disk Encryption), metadata, SSH key configuration
...
* Day0/Day1 configuration for Edge Nodes and Operating system
  ...
  self-contained and don't require network connectivity
...
* Enhanced Lenovo integration

  * Support for LOC-A 3.3.x.
  * OS provisioning can be triggered using |software_prod_name|\
  * Support for meldCX `Viana <https://www.meldcx.com/products/viana>`_  Edge-Init v1.0 Application.

* Support for Intel applications

   * Newest version Pallet defect detection (PDD).
   * Image Based Video Search
   * ChatQnA

* Support for common Helm\* applications

  * A sample set of Bitnami\* applications was selected to validate the orchestrator's compatibility
...
* Enhanced Edge Orchestration deployment and operationalization
...
* Initial Support for Trusted Compute
   ...
   * Provision of a Trusted Compute Capable Edge Node.
...
Refer to the user guide for the detailed documentation of these features.

All of the before mentioned enhancements are present in both AWS-based and
...
You would not be able to upgrade |software_prod_name| to release 3.0 from
...
the guide and then proceeding with a fresh install of release 3.0.
...
* On rare occasions, the Local Volume Manager (LVM) creation of edge nodes
  with Full Disk Encryption (FDE) disabled, fails because it requires input
...
* When a user tries to re-provision without Full Disk Encryption (FDE) a
  node that was previously provisioned with FDE it will not succeed due to
  ...
  without FDE.
* The AAEON UP Squared Pro 7000 provisionign might fail for `Provisioning
...
* Provisioning a node with Ubuntu-ext OS profile through and on-prem based
  orchestrator in an OT network will cause a failure due to squid proxy
  unauthorizing the request with 403. There is no workaround, please
...
* On the host pages, if a host ends its `Under Maintenance` period when a
  ...
  page.
* For Intel :sup:`®`  Core™ processor platforms with E-cores and P-cores,
...
* Support for in-place upgrades of Edge Node Kubernetes cluster versions
  ...
  Currently in 3.0, Cluster upgrade can done by deleting the Cluster and
...
* AI Applications from the earlier release - Scenescape 2024.1.2,
  EIS 2.0_enahanced and Geti 2.6.0 donot work on the 3.0 release. They would
...
* A configurable toggle for Full Disk Encryption (FDE) and Secure Boot (SB)
...
* In a corporate environment where the Product is installed on-premises
  behind a network proxy, the application (DKAM) responsible for obtaining
...
* The embedded JWT tokens in the µOS are programmed to expire after a
...
* The Product does not differentiate P-cores and E-cores in Intel :sup:`®`
  Core™ Processors-based platforms.
* You can create two sites with the same name under two different regions,
  ...
  clusters. It's recommended that sites have unique, non-overlapping names.
...
* Until an Edge Node JWT token is valid, a provisioned Edge Node can reach
  ...
  but it will be rejected by the Orchestrator APIs. Upon token
