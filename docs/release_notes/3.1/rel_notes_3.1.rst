...
* When multiple Ubuntu OS profiles are available, the Orch-CLI tool may select an incorrect profile during
  edge node provisioning. As a workaround, use the UI to register the edge node with the desired Ubuntu OS profile,
  or delete the unwanted Ubuntu OS profiles and then use the Orch-CLI tool to provision the edge node.
