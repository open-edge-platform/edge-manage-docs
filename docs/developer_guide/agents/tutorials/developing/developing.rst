...
An Edge Node supports two types of Operating Systems - Ubuntu* and Edge Microvisor Toolkit. All Edge Node agents support deployment on both types of OS.
...
In cases where the paths or files differ between the OS'es the difference should be handled within the specification and/or the configuration
files of the `debian` or `rpm` package (i.e., Edge Node Agent systemctl service files for each OS).

Occasionally while the overarching functionality provided by an Edge Node Agent may be the same across OS'es the underlying functionality/implementation
