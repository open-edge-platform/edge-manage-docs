...
- Optional destination server private CA certificate is configured but has not been provided.
  ...
  .. note::
    ...
    For both AWS* Cloud and On-premise Edge Orchestrator deployments, the secrets related to SRE destination
...
.. note::
  ...
  If the `sre-exporter` application does not exist or no pods are found, verify that the `sre-exporter` application is enabled in the Edge Orchestrator cluster configuration by:
  ...
  If the profile is not included, an update of the Edge Orchestrator cluster configuration is required to enable the `sre-exporter` service.
