...
are located behind NAT or FW, making direct communication from the Edge Orchestrator
to the edge K8s control plane challenging. This is especially true in cloud
deployment scenarios than on-prem where Edge Orchestrator is located in the cloud
and edges on-premises. To address this problem, Edge Connect Gateway and Agent
provide a secure WebSocket based tunnel between the Edge Orchestrator and edge K8s
...
Cluster Connect Gateway is a microservice running from the Edge Orchestrator cluster
