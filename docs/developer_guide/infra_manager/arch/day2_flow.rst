...
- **Edge Microvisor Toolkit** — An immutable OS in which software is installed on a read-only partition (Partition A).
...
- **Inventory** — Stores the Edge Node instances and the OS Resources related to each instance. 
...
- **Platform Update Agent (PUA)** — A Bare Metal Agent installed on the Edge Node, it is 
...
- **Maintenance Manager** — The Resource Managers responds to status updates from the PUA with 
...
- **OS Resource Manager** — The Resource Manager periodically queries the Release Service 
...
This allows for usecases where the latest available Edge Microvisor Toolkit version may not be desirable,
