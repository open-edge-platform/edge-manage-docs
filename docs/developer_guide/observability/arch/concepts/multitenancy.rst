...
All data ingested and processed by the Observability system operate under the
...
* All metrics and logs contain a **projectId** label (matching a globally
  unique projectId from the data model) to select and visualize data in the UI
  and properly route generated Alerts.

* All metrics and logs sent from the :doc:`/developer_guide/agents/index` are
  shipped with a **X-Scope-OrgID** HTTP header carrying the Edge Node's
  assigned tenant identifier (equal to **projectId**).
...
* to access metrics via the ``orch-cli``.
...
.. note::
   ...
   wildcard-based queries of tenants - applies to ``Grafana`` UI datasources.
   This functionality is provided by the ``grafana-proxy`` service using data
   provided by ``Observability Tenant Controller``.
