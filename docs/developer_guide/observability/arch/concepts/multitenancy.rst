...
- **Orchestrator Observability** which focuses on receiving, processing, and
  querying the data originating from the Orchestrator itself (including the
  Edge Node Observability stack), and providing an overall administrative view
  of Orchestrator and the Edge Nodes.
...
* All metrics and logs sent from the bare-metal agents (BMA) are shipped with
...
on the Orchestrator. It is a single, stateless instance that gracefully handles
