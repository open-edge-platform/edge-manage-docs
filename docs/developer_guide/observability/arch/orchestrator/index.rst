...
* ``Grafana Tempo`` - used as tracces backend.
...
.. note::
   ...
   section for more details on host multitenancy in observability is handled.
...
The stack is separated into following applications:

* ``prometheus-crd`` - provides Prometheus CRDs for other applications to
