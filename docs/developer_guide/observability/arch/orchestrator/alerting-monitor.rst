...
* Integrates with other ``Edge Orchestrator Services`` (``Identity and Access
  Management System``, ``Infrastructure Management`` or ``Grapahical User
  Interface``).
...
multitenacy in observability is handled.
...
Alertmanager email config schema
...
.. image:: ./images/synchronous_api_call_example_diagram.svg
   :alt: Synchronous API call handling example
   :align: center
   :width: 100%
